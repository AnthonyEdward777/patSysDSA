...
class HospitalApp(ctk.CTk):
    def __init__(self):
        ...
        self.btn_sort = ctk.CTkButton(
            self.sidebar,
            text="Sort by Severity (High → Low)",
    ...
    def admit_patient_event(self):
        try:
            ...
            history = self.entry_history.get().strip()
            room_str = self.entry_room.get().strip()
            severity_str = self.entry_severity.get().strip()

            if not name or not age_str or not severity_str:
                messagebox.showerror("Error", "Please fill in basic data (Name, Age, Severity)")
                return

            if not age_str.isdigit() or (room_str and not room_str.isdigit()) or not severity_str.isdigit():
